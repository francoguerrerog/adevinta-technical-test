...
// MARK: - RemoveRandomUser
open class RemoveRandomUserMock: RemoveRandomUser, Mock {
    init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        SwiftyMockyTestObserver.setup()
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
    ...
    open func execute(_ randomUser: RandomUser) -> Single<RandomUsers> {
        addInvocation(.m_execute__randomUser(Parameter<RandomUser>.value(`randomUser`)))
		let perform = methodPerformValue(.m_execute__randomUser(Parameter<RandomUser>.value(`randomUser`))) as? (RandomUser) -> Void
		perform?(`randomUser`)
		var __value: Single<RandomUsers>
		do {
		    __value = try methodReturnValue(.m_execute__randomUser(Parameter<RandomUser>.value(`randomUser`))).casted()
		} catch {
			onFatalFailure("Stub return value not specified for execute(_ randomUser: RandomUser). Use given")
			Failure("Stub return value not specified for execute(_ randomUser: RandomUser). Use given")
		}
		return __value
    }
    ...
    fileprivate enum MethodType {
        case m_execute__randomUser(Parameter<RandomUser>)

        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Bool {
            switch (lhs, rhs) {
            case (.m_execute__randomUser(let lhsRandomuser), .m_execute__randomUser(let rhsRandomuser)):
                guard Parameter.compare(lhs: lhsRandomuser, rhs: rhsRandomuser, with: matcher) else { return false } 
                return true 
            }
        }
        ...
        func intValue() -> Int {
            switch self {
            case let .m_execute__randomUser(p0): return p0.intValue
            }
        }
    }
    ...
    open class Given: StubbedMethod {
        ...
        public static func execute(_ randomUser: Parameter<RandomUser>, willReturn: Single<RandomUsers>...) -> MethodStub {
            return Given(method: .m_execute__randomUser(`randomUser`), products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        public static func execute(_ randomUser: Parameter<RandomUser>, willProduce: (Stubber<Single<RandomUsers>>) -> Void) -> MethodStub {
            let willReturn: [Single<RandomUsers>] = []
			let given: Given = { return Given(method: .m_execute__randomUser(`randomUser`), products: willReturn.map({ StubProduct.return($0 as Any) })) }()
			let stubber = given.stub(for: (Single<RandomUsers>).self)
			willProduce(stubber)
			return given
        }
    ...
    public struct Verify {
        ...
        public static func execute(_ randomUser: Parameter<RandomUser>) -> Verify { return Verify(method: .m_execute__randomUser(`randomUser`))}
    }
    ...
    public struct Perform {
        ...
        public static func execute(_ randomUser: Parameter<RandomUser>, perform: @escaping (RandomUser) -> Void) -> Perform {
            return Perform(method: .m_execute__randomUser(`randomUser`), performs: perform)
        }
    }
